...
for (i in seq_len(nrow(regions))) {
  ...
  qc_thresholds_df <- tibble(
    metric = factor(
      c("Mean STR Qual", "Q10 STR Qual", "STR Error Rate", "STR Ref. Divergence"),
      levels = c("Mean STR Qual", "Q10 STR Qual", "STR Error Rate", "STR Ref. Divergence")
    ),
    threshold = c(
  ...
  qc_plot <- reads_region_df %>%
    pivot_longer(
      cols = c(mean_str_qual, q10_str_qual, str_error_rate, str_ref_divergence),
      names_to = "metric",
      values_to = "value"
    ) %>%
    mutate(
      metric = factor(metric,
        levels = c("mean_str_qual", "q10_str_qual", "str_error_rate", "str_ref_divergence"),
        labels = c("Mean STR Qual", "Q10 STR Qual", "STR Error Rate", "STR Ref. Divergence")
      )
    ) %>%
    ggplot(aes(x = haplotype, y = value, col = haplotype)) +
    geom_hline(
      data = qc_thresholds_df,
    ...
    labs(x = NULL, y = NULL) +
    guides(col = guide_legend(title = "Haplotype")) +
    scale_color_manual(values = HAPLOTYPE_COLORS) +
    theme(
