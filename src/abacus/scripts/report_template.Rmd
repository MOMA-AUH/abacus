...
params:
  ...
  min_mean_str_quality: 20
  min_q10_str_quality: 15
  max_error_rate: 0.01
  max_ref_divergence: 0.34
---
...
for (i in seq_len(nrow(regions))) {
  ...
  # Add notice at top of report
  cat("\n\n")
  cat(paste0(
    '<div class="alert alert-warning" role="alert" style="border-left: 5px solid #f0ad4e; background-color: #fff8e1; padding: 12px 16px;">',
    '<strong>&#9888; Notice:</strong> This report includes all genes analyzed by Abacus. ',
    'Please only review the gene(s) you have been given permission for, i.e., the gene(s) that were requested.',
    '</div>'
  ))

  # Create heading - Note this is a tab
  # Home tab: shown before the first gene tab
  if (i == 1) {
    cat("\n\n")
    cat('## Home')
    cat("\n\n")
    cat(paste0(
      ...
      'Use the tabs above to navigate between genes. Each gene tab contains:\n\n',
      '- A read-level STR plot (coloured by observed k-mer, reference k-mer, or methylation probability)\n',
      '- Tables summarizing the STR structure, read counts for each haplotype\n',
      '- Plot and table showing the QC filtered reads, and the reasons for filtering\n',
      '- A plot showing the consensus STR structure, with the repeat counts for each haplotype\n',
      '- A table summarizing the statistical test results for each haplotype, including confidence intervals of model parameters\n',
      '- An IGV link to open the locus directly in a local IGV session\n'
    ))
    cat("\n\n")
  }
  cat("\n\n")
  ...
  cat("A histogram showing the number of occurences of the individual $k$-mers. If more than one $k$-mer is present in the pattern one histogram is shown for each $k$-mer. Bars are coloured by haplotype; outlier reads are included and shown in purple. QC filtered reads are not shown in the grouped plot.")
