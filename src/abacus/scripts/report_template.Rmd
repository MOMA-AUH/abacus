...
data.frame(
  ...
  column_spec(1, background = "grey98", bold = T)
```

# Test summary

```{r Summary Table, results = TRUE}
# TODO: Only show relevant estiamte! Filter on siginificant?
...
par_summary_df %>%
  ...
  arrange(locus_id, haplotype, idx) %>%
...
for (i in seq_len(nrow(regions))) {
  ...
  data.frame(
    ...
    outliers = nrow(reads_region_df %>% filter(haplotype == "outlier")),
    haplotyped_reads = nrow(reads_region_df %>% filter(haplotype != "outlier"))
  ) %>%
    select(
