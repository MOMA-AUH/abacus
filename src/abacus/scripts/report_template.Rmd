...
create_kmer_plot <- function(kmer_df, color_by, kmer_color_palette, shade_by_qual = FALSE, facet_by_strand = FALSE) {
  ...
  if (facet_by_strand) {
    kmer_plot_df <- kmer_plot_df %>%
      mutate(haplotype_strand = factor(paste(haplotype, strand))) %>%
      group_by(haplotype_strand) %>%
      mutate(read_id = as.integer(factor(read_id))) %>%
      ungroup()
    facet_var <- "haplotype_strand"
  } else {
    facet_var <- "haplotype"
  }
  ...
  hz_lines_df <- kmer_plot_df %>%
    distinct(read_id, alignment_type_txt, start, end, .data[[facet_var]])
  ...
  vertical_fixer_df <- kmer_plot_df %>%
    distinct(read_id, .data[[facet_var]]) %>%
...
summary_table_df <- signif_par_summary_df %>%
  left_join(
    clustering_summary_df %>% select(locus_id, satellite, haplotype, idx, n),
    by = join_by(locus_id, satellite, haplotype, idx)
  ) %>%
  left_join(removed_counts_df, by = join_by(locus_id)) %>%
  mutate(removed_count = if_else(is.na(removed_count), 0L, as.integer(removed_count))) %>%
  arrange(locus_id, idx, haplotype) %>%
  select(locus_id, removed_count, satellite, haplotype, n, mean, mean_lower, mean_upper)
```

## Summary

Count estimates and 95% confidence intervals for all analysed loci. The column "Removed count" shows the number of reads that were removed during QC filtering and length outlier detection. Click a Locus ID to jump to the corresponding section in the report. 
...
for (i in seq_len(nrow(regions))) {
  ...
  # Calculate coverage
  n_haplotyped_reads <- nrow(reads_region_df %>% filter(haplotype %in% c("h1", "h2", "hom")))
  ...
  cat("\n\n")
  cat("#### Sequence (detailed)")
  cat("\n\n")

  if (no_haplotype_reads) {
    cat("There are no haplotyped reads for this locus.")
  } else {
    kmer_plot <- create_kmer_plot(kmer_haplotyped_df, "kmer", kmer_color_palette, shade_by_qual = TRUE, facet_by_strand = TRUE)
    print(kmer_plot)
  }
  ...
  cat("\n\n")
  # TODO: Call haplotype estimates all places
  cat("### Haplotype estimates")
  cat("\n\n")

  # Get coverage per haplotype for this region (for summary table)
  coverage_df <- clustering_summary_region_df %>%
    select(haplotype, n) %>%
    distinct()

  signif_par_summary_region_df %>%
    # Add coverage
    left_join(coverage_df, by = join_by(haplotype)) %>%
    # Round to 2 decimals
    mutate_if(is.numeric, ~ round(., 2)) %>%
    # Arrange by satellite and haplotype
    arrange(haplotype, idx) %>%
  ...
  cat("\n\n")
  cat("#### QC Filtered and outliers (N=", n_qc_filtered_reads + n_outlier_reads, ")", sep = "")
  cat("\n\n")

  if (n_qc_filtered_reads + n_outlier_reads == 0) {
    cat("There are no QC filtered reads for this locus.")
  } else {
    reads_region_df %>%
      filter(haplotype %in% c("qc_filtered", "outlier")) %>%
