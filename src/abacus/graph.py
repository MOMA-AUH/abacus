...
def get_reads_in_locus(bamfile: pysam.AlignmentFile, locus: Locus) -> List[Read]:
...
def graph_align_reads_to_locus(
    ...
    for read in reads:
        ...
        if aln.alignment_type in [
            AlignmentType.LEFT_FLANKING,
            AlignmentType.RIGHT_FLANKING,
        ]:
    ...
    return spanning_alignments, flanking_alignments, filtered_reads, unmapped_reads


def graph_align_flanking_reads_to_locus(flanking_reads: List[GraphAlignment], locus: Locus) -> Tuple[List[GraphAlignment], List[GraphAlignment]]:
    # Initialize lists
    ...
    for read in synthetic_reads:
        ...
        spanning_reads.append(aln)

    return spanning_reads, filtered_reads


def handle_spanning_reads(spanning_reads: List[GraphAlignment], locus: Locus) -> List[ReadCall]:
    # Initialize lists
...
def handle_flanking_reads(flanking_reads: List[GraphAlignment], locus: Locus) -> Tuple[List[ReadCall], List[GraphAlignment]]:
    # Process the results
    filtered_reads: List[GraphAlignment] = []
    read_calls: List[ReadCall] = []
    for aln in flanking_reads:
        # Reads with errors
        if aln.get_error_flags():
            filtered_reads.append(aln)
        ...
        # print("\n\n\n")
        # print(f"Read: {aln.name}\n")
        # print(f"STR error rate: {aln.str_match_ratio}")
        # print(f"Observed kmer string:\n{observed_satellite_string}")
        # print(f"Expected kmer string:\n{expected_satellite_string}")
